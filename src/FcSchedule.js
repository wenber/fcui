...
define(function (require) {
     ...
     * @return {string} html片段
     */
    function getBodyHourHtml(schedule) {
        var me = schedule;
        var html = [];
        var hourHClass = getClass(me, 'hour-head');
        var hourHId = getId(me, 'hour-head');
     ...
     * @return {DOM} tipElement 提示层DOM元素
     */
     ...
     * drag后，重绘选中的值
     * @param {Schedule} schedule Schedule实例
     * @param {Object} cellPos 选中单元块的位置信息
     */
     ...
     * @return {Array} value rawValue的拷贝
     */
    ...
    proto = _.extend(proto, {
        ...
        initEvents: function () {
            ...
            // 绑定选择星期事件
            this.helper.addDOMEvent(lib.g(getId(this, 'day-head')),
                'click', dayClickHandler);
            // 绑定选择时间事件
            this.helper.addDOMEvent(lib.g(getId(this, 'hour-head')),
                'click', hourClickHandler);
            ...
            // shortcut mouseover
            this.helper.addDOMEvent(shortcut,
                'mouseover', lib.curry(shortcutOverOutHandler, true));

            // shortover mouseout
            this.helper.addDOMEvent(shortcut,
                'mouseout', lib.curry(shortcutOverOutHandler, false));
    ...
    Schedule = oo.derive(InputControl, proto);
